...
module.exports = (config) => {
  ...
  const unInstallPackages = packages.filter(name => !Object.keys(dependencies).includes(name))

  const commands = unInstallPackages.length
    ? `npm install --prefix ${baseDir} ${unInstallPackages.join(' ')}`
    : ''
